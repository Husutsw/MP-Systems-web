# 7- ESTADO DE RESULTADOS (ESTILO AJUSTADO AL MODULO 6)
...
def ejecutar_modulo(app):
    ...
    tk.Label(
        contenedor,
        text=f"Estado de Resultados - {app.empresa_actual}",
        font=("Arial", 16, "bold")
    ...
    def agregar_fila():
        row = len(filas) + 2
        tipo = ttk.Combobox(contenedor, values=["Ingreso", "Egreso"], state="readonly", width=17)
        tipo.set("Ingreso")
    ...
    def calcular():
        ...
        msg = f"Ingresos: {app.simbolo_moneda} {ingreso:,.2f} | Egresos: {app.simbolo_moneda} {egreso:,.2f}\n"
        msg += f"Resultado: {'UTILIDAD' if resultado >= 0 else 'PERDIDA'} {app.simbolo_moneda} {abs(resultado):,.2f}"
    ...
    def exportar_pdf():
        ...
        c.drawString(50, 800, f"Estado de Resultados - {app.empresa_actual}")
        y = 770
